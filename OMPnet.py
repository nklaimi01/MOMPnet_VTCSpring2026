...
nb_atoms=len(BS_DoA)
nb_obs=10_000
# nb_obs_list=np.linspace(300,10_000,5,dtype=int)
nb_obs_list=np.array([ 1000,  2725,  5150,  7575, 10000])
means_list=[]
...
for nb_obs in nb_obs_list:
    a_idx=torch.randint(nb_atoms,(nb_obs,4))
    a_val=torch.randn(a_idx.shape,dtype=torch.complex128)+1j*torch.randn(a_idx.shape,dtype=torch.complex128)
    alpha=torch.zeros((nb_obs,nb_atoms),dtype=a_val.dtype)
    alpha.scatter_(1,a_idx,a_val)
    real_D=real_BS_Dictionary
    Hm=alpha@real_D.T

    # Compute noise variance
    nb_elements = Hm.shape[1:].numel()
    sigma2 = (Hm.abs().square().sum(dim=1).mean()) / (nb_elements * snr_avg_lin)
    # Generate complex Gaussian noise
    noise = torch.sqrt(sigma2 / 2) * (torch.randn(*Hm.shape) + 1j * torch.randn(*Hm.shape))
    ...
    # #data visualization:
    # for o in range(5):
    #     plt.plot(torch.abs(real_D.H@Hm[o]))
    #     plt.vlines(torch.nonzero(alpha[o]),0,0.6,colors='red')
    #     plt.show()
    #-------------------------------Get train, validation and test data -------------------------------------------------
    train_test_ratio=0.8
    tt_split_index=int(Hm.shape[0] * train_test_ratio)
    Hm_aux=Hm[:tt_split_index].to(device)
    Ym_aux=Ym[:tt_split_index].to(device)

    # test data 
    Hm_test=Hm[tt_split_index:].to(device)
    Ym_test=Ym[tt_split_index:].to(device)

    #train data
    train_valid_ratio = 0.8
    tv_split_index = int(Hm_aux.shape[0] * train_valid_ratio)
    Hm_train   = Hm_aux [:tv_split_index].to(device)
    Ym_train   = Ym_aux [:tv_split_index].to(device)
    # validation data 
    Hm_val     = Hm_aux [tv_split_index:].to(device)
    Ym_val     = Ym_aux[tv_split_index:].to(device)
    ...
    with torch.no_grad():
        H_test_nominaldict = Ym_test - OMPnet(Ym_test, sigma2,iter_max=iter_max)[0] #Y-r
    ...
    train_losses_list.append(NMSE(Hm_train,Ym_train))
    valid_losses_list.append(NMSE(Hm_val,Ym_val))
    batch_size = 300
    ...
    D_MOD=MOD(Ym_train,D0,OMP_iter=iter_max,epsilon=2e-2,iter_max=2000)
